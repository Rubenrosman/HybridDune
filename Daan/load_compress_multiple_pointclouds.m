...
% V4: save as video
% V5: as V3b, cleaned
map = 'O:\2024-12-18 to 2024-12-20, Storm 1\Lidars\20241220_LiDAR1\2024_12_19_18hr\';
map_save = 'C:\Matlab\hybrid_dunes\lidar\export_tests\test2\';
list_files = dir(map);
...
%% Load pointclouds
r_reshaped_all = zeros(720,16,3,n_max,'uint16');
I_reshaped_all = zeros(720,16,3,n_max,'uint8');
parfor n = n_1 : n_max

    map_naam = [map,list_files(n).name];
    [r_reshaped, I_reshaped,hoek_profile,hoek_ini_at_profile] = load_pointcloud_2(map_naam);
    r_reshaped_all(:,:,:,n) = r_reshaped;
    I_reshaped_all(:,:,:,n) = I_reshaped;
...
r_reshaped_all = r_reshaped_all(:,:,:,n_1:n_max);
I_reshaped_all = I_reshaped_all(:,:,:,n_1:n_max);
...
%% save
% change order of dimensions, to reduce file size
radius = permute(r_reshaped_all,[4 1 2 3]); % matrix of t x 720 x 16 x 3, for time x angles x profile_angles x echos
intensity = permute(I_reshaped_all,[4 1 2 3]);   % note: compressiion works best is similar values are close together. so dimension 1=time, dim2 = 0.5° steps

save([map_save,'test 4D order4123.mat'], 'radius','intensity', 'datetime_all','list_files','angle_profile','angle_ini_at_profile')
% save([map_save,'test 4D order4123.mat'], 'radius','intensity', 'datetime_all','list_files','angle_profile','angle_ini_at_profile','-nocompression') % smaller than above, after zipping
disp('saved')

%% fucntion load_pointcloud_2
function [r_reshaped,  I_reshaped,angle_profile,angle_ini_at_profile] = load_pointcloud_2(map_naam)
% Load point cloud ----------------------------------------------------------------------------------------------------
